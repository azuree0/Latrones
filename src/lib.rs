...
impl GameState {
    ...
    // Rendering methods - moved from JavaScript to Rust
    #[wasm_bindgen]
    pub fn render_game_info_html(&self) -> String {
        let current_player_name = match self.current_player {
            Player::Light => "Light",
            Player::Dark => "Dark",
        };
        
        let phase_display = if self.game_over {
        ...
        let player_class = if self.current_player == Player::Dark { "dark" } else { "" };
        
        format!(
            r#"<div class="game-info">
      ...
      <div class="controls">
        <button class="btn btn-primary" onclick="window.resetGame()">
          Reset
        </button>
        <button class="btn btn-secondary" onclick="window.setStartingPieces()">
          Set
        </button>
      </div>
    </div>"#,
            player_class, current_player_name, phase_display
        )
    }
    
    #[wasm_bindgen]
    pub fn render_game_board_html(&self) -> String {
        let board: Vec<u8> = self.board.iter().map(|sq| {
            match sq {
                Square::Empty => 0,
                Square::Piece(Player::Light) => 1,
                Square::Piece(Player::Dark) => 2,
        ...
        // Get valid moves directly without going through JsValue
        ...
        match self.phase {
            ...
            GamePhase::Movement => {
                if let Some(selected) = self.selected_square {
                    if matches!(self.board[selected], Square::Piece(p) if p == self.current_player) {
                        valid_moves = self.get_valid_destinations(selected);
                    }
                } else {
                    for i in 0..64 {
                        if let Square::Piece(p) = self.board[i] {
        ...
        let rank_labels = ['1', '2', '3', '4', '5', '6', '7', '8'];
        
        let mut html = String::from(r#"<div class="board-container">"#);
        
        // Top labels
        html.push_str(r#"<div class="board-labels top-labels">"#);
        html.push_str(r#"<div class="label-corner"></div>"#);
        for label in file_labels.iter() {
            html.push_str(&format!(r#"<div class="board-label file-label">{}</div>"#, label));
        }
        html.push_str(r#"<div class="label-corner"></div>"#);
        html.push_str("</div>");
        
        // Board
        html.push_str(r#"<div class="board">"#);
        for (row_index, rank) in rank_labels.iter().enumerate() {
            html.push_str(r#"<div class="board-row">"#);
            html.push_str(&format!(r#"<div class="board-label rank-label">{}</div>"#, rank));
            html.push_str(r#"<div class="board-squares">"#);
            
            for (col_index, _file) in file_labels.iter().enumerate() {
                let square_index = row_index * 8 + col_index;
                let square_value = board[square_index];
                
                let mut classes = String::from("square");
                
                match square_value {
                    0 => classes.push_str(" empty"),
                    1 => classes.push_str(" light-piece"),
                    2 => classes.push_str(" dark-piece"),
                    _ => {}
                }
                
                if valid_moves.contains(&square_index) {
                    classes.push_str(" valid-move");
                }
                
                if selected_square == Some(square_index) {
                    classes.push_str(" selected");
                } else if square_value == 1 || square_value == 2 {
                    let piece_player = if square_value == 1 { Player::Light } else { Player::Dark };
                    if piece_player == self.current_player && valid_moves.contains(&square_index) {
                        classes.push_str(" selectable");
                    }
                }
                
                let content = match square_value {
                    1 => "○",
                    2 => "●",
                    _ => "",
                };
                ...
                html.push_str(&format!(
                    r#"<div class="{}" onclick="window.selectSquare({})">{}</div>"#,
                    classes, square_index, content
                ));
            }
        ...
        // Bottom labels
        html.push_str(r#"<div class="board-labels bottom-labels">"#);
        html.push_str(r#"<div class="label-corner"></div>"#);
        for label in file_labels.iter() {
            html.push_str(&format!(r#"<div class="board-label file-label">{}</div>"#, label));
        }
